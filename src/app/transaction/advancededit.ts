...
@Component({
  selector: 'app-advancededit',
  templateUrl: './advancededit.html',
  styleUrls: ['./advancededit.scss'],
})
export class AdvancedEditComponent {
  ...
  setData(item: TxItem, accountTree: AccountTree) {
    ...
    this.selectAccounts = accountTree
      ...
      let control = new FormGroup(
        {
          ...
          debitNative: new FormControl(
            split.nativeAmount >= 0
              ? split.nativeAmount / Math.pow(10, orgPrecision)
              : null,
          ),
  ...
  submit() {
    ...
    for (let i = 0; i < this.form.value.splits.length; i++) {
      ...
      let nativeAmount = split.debitNative
        ? parseFloat(split.debitNative)
        : -parseFloat(split.creditNative);
      nativeAmount = Math.round(
        nativeAmount * Math.pow(10, this.org.precision),
      );
